...
variable "opensearch_zone_awareness_enabled" {
  type        = bool
  default     = false
  description = "Enable zone awareness for HA."
}
...
variable "opensearch_vpc_subnet_ids" {
  type        = list(string)
  default     = []
  description = "VPC subnet IDs (1, 2, or 3 subnets)."
}
...
variable "opensearch_enforce_https" {
  type        = bool
  default     = true
  description = "Force HTTPS for domain endpoint."
}

variable "opensearch_tls_security_policy" {
  type        = string
  default     = "Policy-Min-TLS-1-2-2019-07"
  description = "TLS policy for domain endpoint."
}

variable "opensearch_master_user_name" {
  type        = string
  default     = ""
  description = "Master user (optional; enables FGAC if set with password)."
}
